...
disp_dens <- ggplot2::ggplot(
    resid_df %>%
        dplyr::filter(disp_type2 != "Total Disparity"),
    ggplot2::aes(x = p500, color = model_name)
) +
    ...
    ggplot2::scale_x_continuous("County median estimate", expand = c(0, 0)) +
    ggplot2::scale_y_continuous(NULL, expand = c(0, 0)) +
    ggplot2::labs(title = NULL)
ggplot2::ggsave(
    './plots/disparity_res_density.pdf',
    disp_dens,
    height = 3,
    width = 6,
    scale = 1.25
)
saveRDS(disp_dens, "./grobs/density_of_residual_disp.RDS")
...
disp_map <- ggplot2::ggplot(data = resid_df) +
    ...
    ggplot2::theme_classic() +
    ggplot2::scale_fill_brewer("Median spatially-varying disparity",
                               palette = "Spectral",
                               direction = -1) +
    ggplot2::theme(
    ...
    ggplot2::labs(title = NULL) + #"Spatially-varying disparity") +
    ggplot2::guides(
...
ggplot2::ggsave(
    disp_map,
    file = './plots/disp_maps_categorized.jpg',
    width = 6,
    height = 7,
    scale = 1.25,
    dpi = 300
...
ggplot2::ggsave(
    disp_map,
    file = './plots/disp_maps_categorized.pdf',
    width = 6,
    height = 7,
    scale = 1.25,
...
disp_map_sig <- ggplot2::ggplot(data = resid_df) +
    ...
    ggplot2::scale_fill_brewer(
        "Median spatially-varying disparity",
        palette = "Spectral",
        direction = -1,
        na.value = "grey50"
    ) +
...
ggplot2::ggsave(
    disp_map_sig,
    file = './plots/disp_maps_categorized_sig.jpg',
    width = 6,
    height = 7,
    scale = 1.25,
    dpi = 300
...
ggplot2::ggsave(
    disp_map_sig,
    file = './plots/disp_maps_categorized_sig.pdf',
    width = 6,
    height = 7,
    scale = 1.25,
    device = grDevices::cairo_pdf
